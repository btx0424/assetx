...
def main() -> None:
    ...
    parser.add_argument("--path", "-p", type=str, required=True, help="Path to the URDF file.")
    parser.add_argument(
        "--output",
        "-o",
        type=str,
    ...
    spec.compile()
    out_path = Path(args.path).with_suffix(".xml") if args.output is None else Path(args.output)
